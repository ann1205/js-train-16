...
function filterStudentsByGrade(students, grade) {
  ...
  const studentsWithGrade = students.filter(
...
function compareArrays(arr1, arr2) {
  ...
  const json1 = JSON.stringify(sortedArray1);
  const json2 = JSON.stringify(sortedArray2);
...
function displayGroupedInfo(students) {
  ...
  for (const stud of students) {
    console.group(`Студент: ${stud.name}`);
    console.info(`Вік: ${stud.age}`);
    console.info(`Предмет: ${stud.subject}`);
    console.groupEnd();
  }
}
